...
// ─── Persistent storage ───────────────────────────────────────────────────────
function useStore() {
  const load = (k, d) => { try { const v = window._store?.[k]; return v !== undefined ? v : d; } catch { return d; } };
  ...
  const [state, setState] = useState({ ...window._store });
  const update = (patch) => {
    window._store = { ...window._store, ...patch };
    setState({ ...window._store });
  };
  return [state, update];
}
...
.btn {
  font-family: 'DM Sans', sans-serif; font-weight: 700; border: none; cursor: pointer;
  border-radius: 99px; transition: transform .15s, box-shadow .15s, opacity .15s;
...
input, textarea, select {
  font-family: 'DM Sans', sans-serif; font-size: 16px; width: 100%;
  border: 2px solid #EEE; border-radius: 14px; padding: 14px 16px;
  background: white; color: ${C.ink}; outline: none; transition: border-color .2s;
...
.nl-italic   { font-family: 'Playfair Display', serif; font-style: italic; }
`;
...
const Confetti = ({ active }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:9999, overflow:"hidden" }}>
      {pieces.map((p,i) => (
        <div key={i} style={{ position:"absolute", top:"5%", left:`${p.left}%`, width:p.size, height:p.size, borderRadius:2, background:p.color, opacity:0, animation:`confetti 2s ease forwards`, animationDelay:`${p.delay}s` }} />
      ))}
    </div>
  );
};
...
// ─── Image uploader ───────────────────────────────────────────────────────────
function ImageUploader({ value, onChange, multiple=false, label="Upload photo", accept="image/*" }) {
  const ref = useRef();
  const handleFiles = (files) => {
    ...
    Promise.all(arr.map(f => new Promise(res => {
      ...
      r.readAsDataURL(f);
    }))).then(results => onChange(multiple ? results : results[0]));
  };
  const imgs = multiple ? (value || []) : (value ? [value] : []);
  return (
    <div>
      <div onClick={() => ref.current.click()} style={{ border:`2px dashed ${C.pink}66`, borderRadius:16, padding:"28px 20px", textAlign:"center", cursor:"pointer", background:`${C.pink}08`, transition:"background .2s", WebkitTapHighlightColor:"transparent", minHeight:100, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
...
function HomeScreen({ state, onNavigate }) {
  const { friends, submissions } = state;
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", paddingBottom:40 }}>
      ...
      <div style={{ background:C.ink, padding:"max(40px, env(safe-area-inset-top)) 24px 32px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", top:20, right:24, width:60, height:60, borderRadius:"50%", background:C.mint, opacity:.1 }} />
        ...
        <p style={{ color:"rgba(255,255,255,.6)", fontSize:14, marginTop:6 }}>
          {allDone ? "🎉 Everyone's in! Newsletter is ready." : `Waiting for ${total-submitted} more submission${total-submitted!==1?"s":""}...`}
        </p>
        {/* Progress */}
      ...
      <div style={{ padding:"24px 16px 0" }}>
        ...
        <button className="btn" onClick={() => onNavigate("submit")} style={{ width:"100%", padding:"17px", fontSize:16, background:`linear-gradient(135deg,${C.pink},${C.orange})`, color:"white", boxShadow:`0 8px 24px ${C.pink}44`, marginBottom:12 }}>
          ✏️ Submit My Responses
        </button>

        {allDone && (
          <button className="btn" onClick={() => onNavigate("newsletter")} style={{ width:"100%", padding:"17px", fontSize:16, background:`linear-gradient(135deg,${C.blue},${C.purple})`, color:"white", boxShadow:`0 8px 24px ${C.blue}44`, marginBottom:12 }}>
...
function SubmitScreen({ state, update, onBack }) {
  ...
  const handleSubmit = () => {
    const newSubs = { ...submissions, [who.id]: { answers, photos } };
    const newFriends = friends.map(f => f.id===who.id ? { ...f, submitted:true } : f);
    update({ submissions: newSubs, friends: newFriends });
    setStep("done");
  };
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", padding:"max(24px,env(safe-area-inset-top)) 16px 120px" }}>
      ...
      <div style={{ position:"sticky", bottom:0, left:0, right:0, background:`linear-gradient(transparent, ${C.bg} 30%)`, paddingTop:20, paddingBottom:"max(24px,env(safe-area-inset-bottom))", marginTop:8 }}>
        <button className="btn" onClick={handleSubmit} style={{ width:"100%", padding:"20px", fontSize:17, background:`linear-gradient(135deg,${who.color},${C.pink})`, color:"white", boxShadow:`0 8px 24px ${who.color}44` }}>
          Submit My Responses 🚀
        </button>
      </div>
    </div>
...
function NewsletterScreen({ state, update, onBack, isArchive=false, archiveData=null }) {
  const { friends, questions, submissions, editorNote, heroImage } = isArchive ? archiveData : state;
  ...
  const generateSummary = async (q) => {
    ...
    const entries = friends.map(f => {
      const text = (isArchive ? archiveData.submissions : submissions)[f.id]?.answers?.[q.id];
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens:200,
          messages:[{ role:"user", content:`You are writing a fun, warm, witty 1–2 sentence italic theme summary for a section of a friends' newsletter called "Monthly Inbox of Chaos". The summary appears above friends' individual answers and captures the overall vibe/theme of everyone's responses in a playful editorial voice. Keep it under 40 words, make it fun and specific to what they shared. Do NOT use bullet points or names. Just write the summary sentence(s).\n\nSection: "${q.label}"\nFriends' answers:\n${entries}` }]
        })
      });
      const data = await res.json();
      const text = data.content?.find(c=>c.type==="text")?.text || "";
      setSummaries(s => ({ ...s, [q.id]: text.trim() }));
  ...
  const generateAll = async () => {
    ...
    for (const q of questions) {
      await generateSummary(q);
    }
    setAllGenerated(true);
    setGenerating(false);
  };

  const allPhotos = friends.flatMap(f => {
    const subs = isArchive ? archiveData.submissions : submissions;
    return (subs[f.id]?.photos || []).map(p => ({ src:p, friend:f }));
  });
  ...
  return (
    <div style={{ maxWidth:680, margin:"0 auto", background:C.bg }}>
      ...
      {/* Newsletter hero — mirrors the actual newsletter style */}
      <div style={{ margin:"16px 16px 0", borderRadius:24, overflow:"hidden", background:`linear-gradient(135deg, #6A0DAD, #4D96FF, #00C9C8)`, position:"relative", minHeight:220 }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"36px 32px" }}>
          <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:42, color:"white", lineHeight:1, marginBottom:4 }}>Welcome to</div>
          ...
          <div style={{ background:C.yellow, color:C.ink, borderRadius:6, display:"inline-block", padding:"5px 14px", marginTop:14, fontSize:13, fontWeight:700 }}>
            The gang's highlights, fails &amp; fun — all in one drop.
          </div>
        </div>
      </div>

      <div style={{ padding:"28px 24px" }}>
        <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:30, fontWeight:900, marginBottom:20 }}>
          Friends Newsletter — {month}
        </h1>

        {/* Editor's note */}
        {editorNote && (
          <div style={{ marginBottom:32, paddingBottom:28, borderBottom:`1px solid #EEE` }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
              <span style={{ fontSize:18 }}>🌿</span>
              <span style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontWeight:700, fontSize:16 }}>Editor's Note</span>
            </div>
            <p style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", lineHeight:1.8, color:C.ink, fontSize:15, whiteSpace:"pre-wrap" }}>{editorNote}</p>
          </div>
        )}

        {/* Generate all button */}
        {!isArchive && !allGenerated && (
          <div style={{ background:`${C.purple}18`, border:`2px solid ${C.purple}40`, borderRadius:16, padding:"16px 20px", marginBottom:28, display:"flex", alignItems:"center", gap:14, justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
            <div>
              <div style={{ fontWeight:700, fontSize:15 }}>✨ AI Section Summaries</div>
              <div style={{ fontSize:13, color:C.muted }}>Auto-generate the fun italic intros for each section</div>
            </div>
            <button className="btn" onClick={generateAll} disabled={generating} style={{ padding:"10px 20px", fontSize:14, background:C.purple, color:"white", whiteSpace:"nowrap" }}>
              {generating ? <><Spinner /> Generating…</> : "Generate All ✨"}
            </button>
          </div>
        )}
        ...
        {questions.map(q => {
          const subs = isArchive ? archiveData.submissions : submissions;
          const entries = friends.map(f => ({ friend:f, text: subs[f.id]?.answers?.[q.id] })).filter(e=>e.text);
          ...
          return (
            <div key={q.id} style={{ marginBottom:36, paddingBottom:32, borderBottom:`1px solid #EEE` }}>
              ...
              <div style={{ display:"flex", alignItems:"flex-start", gap:10, marginBottom:14, minHeight:30 }}>
                {loadingQ===q.id ? (
                  <div style={{ display:"flex", alignItems:"center", gap:8, color:C.muted, fontSize:14 }}><Spinner /><span>Writing summary…</span></div>
                ) : summary ? (
                  <p style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", color:`#5A3E8A`, fontSize:15, lineHeight:1.7, flex:1 }}>{summary}</p>
                ) : !isArchive ? (
                  <button onClick={() => generateSummary(q)} style={{ background:"none", border:`1.5px dashed ${C.purple}60`, color:C.purple, borderRadius:99, padding:"4px 14px", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                    + Generate summary
                  </button>
                ) : null}
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {entries.map(({ friend, text }) => (
                  <div key={friend.id} style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                    ...
                    <span style={{ fontSize:15, lineHeight:1.65, color:C.ink }}>{text}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {allPhotos.length > 0 && (
          <div style={{ marginBottom:32 }}>
            ...
            <p style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", color:"#5A3E8A", fontSize:14, marginBottom:16 }}>Visual evidence of our adventures... place your bets on who submitted the most chaotic pic 👀</p>
        ...
        {!isArchive && (
          <button className="btn" onClick={() => {
            ...
            const newArchive = [entry, ...(state.archive||[])];
            update({ archive:newArchive, submissions:{}, friends:state.friends.map(f=>({...f,submitted:false})), editorNote:"", heroImage:null });
            onBack();
...
// ─── ARCHIVE SCREEN ───────────────────────────────────────────────────────────
function ArchiveScreen({ state, onBack, onView }) {
  const { archive } = state;
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", padding:"24px 16px 60px" }}>
      ...
      {archive.length===0 ? (
        ...
        <button key={i} onClick={() => onView(entry)} style={{ width:"100%", background:C.card, border:`2px solid ${colors[i%colors.length]}40`, borderRadius:20, padding:"20px", display:"flex", alignItems:"center", gap:14, marginBottom:12, cursor:"pointer", boxShadow:"0 2px 10px rgba(0,0,0,.05)", textAlign:"left", transition:"transform .15s" }}
>
          ...
          <div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:18 }}>{entry.month}</div>
            <div style={{ fontSize:13, color:C.muted }}>{entry.friends?.length} contributors</div>
          </div>
...
function AdminScreen({ state, update, onBack }) {
  ...
  const [tab, setTab] = useState("editor"); // editor | questions | members

  const tabStyle = (t) => ({
    flex:1, padding:"10px", border:"none", cursor:"pointer", borderRadius:10, fontWeight:700, fontSize:13,
    background: tab===t ? C.ink : "transparent", color: tab===t ? "white" : C.muted, transition:"all .2s"
  });
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", padding:"24px 16px 80px" }}>
      ...
      <div style={{ display:"flex", gap:4, background:"#F0F0F0", borderRadius:14, padding:4, marginBottom:24 }}>
        <button style={{...tabStyle("editor"), minHeight:44}} onClick={()=>setTab("editor")}>✏️ Editor</button>
        <button style={{...tabStyle("questions"), minHeight:44}} onClick={()=>setTab("questions")}>❓ Q's</button>
        <button style={{...tabStyle("members"), minHeight:44}} onClick={()=>setTab("members")}>👥 Members</button>
      </div>
      ...
      {tab==="editor" && (
        <div>
          <div style={{ marginBottom:20 }}>
            <label>Editor's Note (Jules only)</label>
            <p style={{ fontSize:13, color:C.muted, marginBottom:8 }}>This appears at the top of the newsletter in italic</p>
            <textarea rows={6} placeholder="Welcome back, friends! Okay… yes. It's been a minute..." value={editorNote} onChange={e=>update({ editorNote:e.target.value })} />
          </div>
          <div style={{ marginBottom:20 }}>
            <label>Hero / Header Photo</label>
            <p style={{ fontSize:13, color:C.muted, marginBottom:8 }}>Group photo that appears in the newsletter header</p>
      ...
      {tab==="questions" && (
        <div>
          <p style={{ fontSize:14, color:C.muted, marginBottom:16 }}>Drag to reorder. Toggle to hide for this month.</p>
          {questions.map((q,i) => (
            <div key={q.id} style={{ background:C.card, borderRadius:16, padding:"14px 16px", marginBottom:10, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 8px rgba(0,0,0,.05)", border:"2px solid #EEE" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:C.muted }}>{q.prompt}</div>
              </div>
              <button onClick={() => update({ questions: questions.filter((_,j)=>j!==i) })} style={{ background:"#FF444422", color:"#FF4444", border:"none", borderRadius:8, padding:"5px 10px", cursor:"pointer", fontWeight:700, fontSize:12 }}>Remove</button>
            </div>
          ))}
          {/* Add new */}
          <div style={{ background:`${C.mint}18`, border:`2px solid ${C.mint}40`, borderRadius:18, padding:"18px", marginTop:16 }}>
            ...
            <div style={{ marginBottom:10 }}>
              <label>Emoji</label>
              <input value={newQ.emoji} onChange={e=>setNewQ(q=>({...q,emoji:e.target.value}))} style={{ width:70 }} />
            </div>
            <div style={{ marginBottom:10 }}>
              <label>Section Label</label>
              <input placeholder="e.g. Best Meal of the Month" value={newQ.label} onChange={e=>setNewQ(q=>({...q,label:e.target.value}))} />
            </div>
            <div style={{ marginBottom:10 }}>
              <label>Question Prompt</label>
              <input placeholder="e.g. What did you eat that slapped this month?" value={newQ.prompt} onChange={e=>setNewQ(q=>({...q,prompt:e.target.value}))} />
            </div>
            <div style={{ marginBottom:14 }}>
              <label>Placeholder Text</label>
              <input placeholder="e.g. Don't say cereal..." value={newQ.placeholder} onChange={e=>setNewQ(q=>({...q,placeholder:e.target.value}))} />
            </div>
            <button className="btn" onClick={() => {
              if (!newQ.label.trim()) return;
              update({ questions: [...questions, { ...newQ, id:"q_"+Date.now() }] });
              setNewQ({ emoji:"💬", label:"", prompt:"", placeholder:"" });
            }} style={{ width:"100%", padding:"13px", fontSize:14, background:C.mint, color:C.ink }}>Add Question</button>
          </div>
        </div>
      )}
      ...
      {tab==="members" && (
        <div>
          {friends.map(f => (
            <div key={f.id} style={{ background:C.card, borderRadius:16, padding:"14px 16px", marginBottom:10, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 8px rgba(0,0,0,.05)" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:13, color:C.muted }}>{f.submitted?"✓ Submitted this month":"Pending"}</div>
              </div>
            </div>
          ))}
          <div style={{ background:"#FFF0F0", border:"2px solid #FFD0D0", borderRadius:18, padding:"18px", marginTop:20 }}>
            ...
            <p style={{ fontSize:14, color:C.muted, marginBottom:14 }}>Use this at the start of each new month to clear everyone's status.</p>
            <button className="btn" onClick={() => update({ friends: friends.map(f=>({...f,submitted:false})), submissions:{} })} style={{ padding:"12px 24px", fontSize:14, background:"#FF4444", color:"white" }}>Reset All Submissions</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ maxWidth:340, margin:"0 auto", padding:"60px 24px", textAlign:"center" }}>
      ...
      <div style={{ display:"flex", justifyContent:"center", gap:14, marginBottom:36, animation: shake ? "shake .4s ease" : "none" }}>
        {[0,1,2,3].map(i => (
          <div key={i} style={{ width:18, height:18, borderRadius:"50%", border:`2.5px solid ${error?"#FF4444":C.pink}`, background: pin.length > i ? (error?"#FF4444":C.pink) : "transparent", transition:"background .15s, border-color .15s" }} />
        ))}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, maxWidth:260, margin:"0 auto" }}>
        {digits.map((d,i) => (
          ...
          <button key={i} onClick={() => d==="\u232b" ? handleDelete() : handleDigit(d)}
            style={{ background:d==="\u232b"?"#F0F0F0":C.card, border:`2px solid ${d==="\u232b"?"#E0E0E0":"#EEE"}`, borderRadius:16, padding:"22px 0", fontSize:d==="\u232b"?22:26, fontWeight:700, cursor:"pointer", color:C.ink, transition:"background .1s", fontFamily:"'DM Sans',sans-serif", minHeight:64, WebkitTapHighlightColor:"transparent" }}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (n === state.friends.length && state.friends.length > 0 && n > prevSubmitted.current) {
      setConfetti(true);
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg }}>
      ...
      {screen==="archive"    && <ArchiveScreen state={state} onBack={()=>nav("home")} onView={e=>{ setArchiveEntry(e); setScreen("newsletter"); window.scrollTo(0,0); }} />}
